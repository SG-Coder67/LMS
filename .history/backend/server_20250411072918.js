...
app.post("/api/auth/register", async (req, res) => {
    ...
    if (!passwordRegex.test(password)) {
        return res.status(400).json({ msg: "Password must contain at least one capital letter and one '$'" });
    }
...
app.get("/api/student/courses", isAuthenticated, isStudent, async (req, res) => {
    try {
        ...
        const enrollments = await Enrollment.find({ student: studentId })
            ...
            .lean(); 
        const courses = enrollments.map(en => en.course).filter(Boolean); 
        res.json(courses);
    } catch (err) {
...
app.post("/api/teacher/courses/:courseId/assignments", isAuthenticated, isTeacher, async (req, res) => {
    ...
    try {
        ...
        const newAssignment = new Assignment({
            ...
            totalMarks: totalMarks ? Number(totalMarks) : 100, // Use provided or default
...
// --- Add this ENTIRE block in server.js ---

// Endpoint to mark a course as completed and unenroll students below attendance threshold
app.post("/api/teacher/courses/:courseId/complete", isAuthenticated, isTeacher, async (req, res) => {
    const courseId = req.params.courseId;
    const teacherId = req.session.user.id;
    const MIN_CLASSES_REQUIRED = 10;
    const MIN_ATTENDANCE_PERCENT = 75;

    try {
        // 1. Find the course and verify ownership & status
        const course = await Course.findOne({ _id: courseId, createdBy: teacherId });
        if (!course) {
            return res.status(404).json({ msg: "Course not found or you are not the creator." });
        }
        // Check if already completed (requires schema change)
        if (course.status === 'completed') {
             return res.status(400).json({ msg: "This course has already been marked as completed." });
        }

        // 2. Check minimum class requirement
        // Find distinct dates where attendance was marked for this course
        const distinctClassDates = await Attendance.distinct('date', { course: courseId });
        const totalClassesHeld = distinctClassDates.length;

        if (totalClassesHeld < MIN_CLASSES_REQUIRED) {
            return res.status(400).json({ msg: `Cannot complete course. Minimum ${MIN_CLASSES_REQUIRED} classes required, but only ${totalClassesHeld} were held (based on unique attendance dates).` });
        }

        // 3. Process enrollments: Calculate attendance and unenroll if needed
        const enrollments = await Enrollment.find({ course: courseId }).populate('student', 'username');
        let unenrolledStudents = [];
        let processedCount = 0;

        for (const enrollment of enrollments) {
            if (!enrollment.student) {
                console.warn(`Enrollment record ${enrollment._id} missing student reference.`);
                continue; // Skip if student data isn't populated correctly
            }
            const studentId = enrollment.student._id;

            // Count PRESENT days for this student in this course
            ...
            const attendancePercentage = totalClassesHeld > 0 ? (presentCount / totalClassesHeld) * 100 : 0;

            console.log(`Student ${enrollment.student.username}: Present ${presentCount}/${totalClassesHeld} (${attendancePercentage.toFixed(1)}%)`);

            if (attendancePercentage < MIN_ATTENDANCE_PERCENT) {
                // Unenroll the student
                await Enrollment.findByIdAndDelete(enrollment._id);
                unenrolledStudents.push(enrollment.student.username);
                console.log(`-- Unenrolling student ${enrollment.student.username}`);
            }
            processedCount++;
        }

        // 4. Mark the course as completed (requires schema change)
         await Course.findByIdAndUpdate(courseId, { status: 'completed' });
         console.log(`Course ${course.name} (${courseId}) marked as completed.`);

        // 5. Respond
        let responseMsg = `Course "${course.name}" marked as completed. ${totalClassesHeld} classes were held. ${processedCount} students processed.`;
        if (unenrolledStudents.length > 0) {
            responseMsg += ` ${unenrolledStudents.length} students unenrolled due to attendance below ${MIN_ATTENDANCE_PERCENT}%: ${unenrolledStudents.join(', ')}.`;
        } else {
            responseMsg += ` All enrolled students met the attendance requirement.`;
        }

        res.json({ msg: responseMsg, unenrolledCount: unenrolledStudents.length, unenrolledStudents: unenrolledStudents });

    } catch (err) {
        console.error(`Error completing course ${courseId}:`, err);
        res.status(500).json({ msg: "Internal server error during course completion." });
    }
});
...
app.listen(PORT, () => {
    ...
    console.warn("⚠️ SECURITY WARNING: Server is running WITHOUT password hashing. Passwords are stored in plain text!");
});